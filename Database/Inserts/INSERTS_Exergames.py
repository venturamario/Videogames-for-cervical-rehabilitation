...
def generate_inserts(num_users, filename):
    faker = Faker()
    with open(filename, 'w', encoding='utf-8') as file:  # Asegúrate de abrir el archivo con la codificación adecuada
        ...
        games = []
        # Bricks Breaker
        insert_command = f"INSERT INTO public.juego(idGame, name, description, difficulty, idCerv) VALUES ({1}, 'Bricks Breaker', 'Un adictivo juego clásico donde debes romper ladrillos usando una bola que rebota en tu paleta. Dirige la bola para destruir todos los ladrillos y avanza a través de niveles desafiantes. ¡No dejes caer la bola o perderás!', {3.0}, {1}) ON CONFLICT (idGame) DO NOTHING;\n"
        file.write(insert_command)
        games.append(1)
        # Pacman
        insert_command = f"INSERT INTO public.juego(idGame, name, description, difficulty, idCerv) VALUES ({2}, 'Pacman', 'Un icónico juego de arcade donde controlas a Pacman en un laberinto lleno de puntos y fantasmas. Come todos los puntos y frutas para ganar puntos, y usa power-ups para hacer que los fantasmas se vuelvan vulnerables. ¡Cuidado con los fantasmas!', {4.5}, {3}) ON CONFLICT (idGame) DO NOTHING;\n"
        file.write(insert_command)
        games.append(2)
        # Flappy Bird
        insert_command = f"INSERT INTO public.juego(idGame, name, description, difficulty, idCerv) VALUES ({3}, 'Flappy Bird', 'Un desafiante juego de habilidad donde controlas a un pájaro que debe volar entre tuberías sin tocarlas. Toca la pantalla para mantener al pájaro en vuelo y trata de superar la mayor cantidad de obstáculos posible. ¡Un solo choque y es el fin!', {3.5}, {2}) ON CONFLICT (idGame) DO NOTHING;\n"
        file.write(insert_command)
        games.append(3)
        # Snake Game
        insert_command = f"INSERT INTO public.juego(idGame, name, description, difficulty, idCerv) VALUES ({4}, 'Snake Game', 'Controla una serpiente que crece al comer comida esparcida por el campo. Guía a la serpiente con cuidado para evitar chocar contra las paredes o su propio cuerpo mientras intentas crecer lo más posible. ¡Cada bocado hace que el juego sea más desafiante!', {3.0}, {3}) ON CONFLICT (idGame) DO NOTHING;\n"
        file.write(insert_command)
        games.append(4)
        ...
        usernames = []
        insert_command = f"INSERT INTO public.usuario(username, name, lastname, password, level, xp) VALUES ('test', 'test', 'test', 'test', {9999}, {99}) ON CONFLICT (username) DO NOTHING;\n"
        file.write(insert_command)
        usernames.append('test')
        insert_command = f"INSERT INTO public.usuario(username, name, lastname, password, level, xp) VALUES ('xisca.roig', 'Xisca', 'Roig', {1234}, {9999}, {99}) ON CONFLICT (username) DO NOTHING;\n"
        file.write(insert_command)
        usernames.append('xisca.roig')
        insert_command = f"INSERT INTO public.usuario(username, name, lastname, password, level, xp) VALUES ('mario.ventura', 'Mario', 'Ventura', '{8522}', {9999}, {99}) ON CONFLICT (username) DO NOTHING;\n"
        file.write(insert_command)
        ...
        for username in usernames:
            for idGame in games:
                ...
                insert_command = f"INSERT INTO public.puntos(numPoints, idGame, username) VALUES ({numPoints}, {idGame}, '{username}') ON CONFLICT (idGame, username) DO NOTHING;\n"
                file.write(insert_command)
...
if __name__ == "__main__":
    generate_inserts(10000, r'C:/Users/Usuario/Desktop/Exergames para rehabilitacion cervical/BD/Inserts/InsertsGenerales.txt')
